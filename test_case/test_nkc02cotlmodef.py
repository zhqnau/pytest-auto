...
from pagelocator.c02_locator import *  # 引用此脚本对应的xpath定位信息
from pagedata.nk_data import NkData  # 引用此脚本对应的测试数据信息
import allure  # @allure修饰器 ，allure报告引用需要
from utils.timenum import nt


@allure.description(f"受控模块生效申请,本次执行时间为：{nt}")  # 对应allure报告描述信息
@allure.feature("受控模块生效申请模块")   # 对应allure报告模块名称
...
class TestControlledModuleEffective():
    ...
    @pytest.mark.parametrize(
        'menuname, applydesc', NkData.TestControlledModuleEffective)
